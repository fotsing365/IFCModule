...
bool CreateGeometricRepresentationVisitor::visitIfcRepresentation(ifc2x3::IfcRepresentation * value)
{
   ...
   if (_colour != 0)
   {
	   // create IFCSURFACESTYLERENDERING
	   ifc2x3::IfcSurfaceStyleRendering *ssr = mDataSet->createIfcSurfaceStyleRendering().get();
	   // associate _colour
	   ssr->setDiffuseColour(_colour);
	   // create IFCSURFACESTYLE
	   ifc2x3::IfcSurfaceStyle *iss = mDataSet->createIfcSurfaceStyle().get();
	   // associate to created IFCSURFACESTYLERENDERING
	   iss->setStyles(ssr);

	   // create IFCPRESENTATIONSTYLEASSIGNMENT

	   ifc2x3::IfcPresentationStyleAssignment *psa = mDataSet->createIfcPresentationStyleAssignment().get();
	   // associate to created IFCSURFACESTYLE
	   

	   psa->getStyles().insert(iss);
	   //styleAss->setStyles(style);

	   // create IFCSTYLEDITEM

	   ifc2x3::IfcStyledItem *isi = mDataSet->createIfcStyledItem().get();

	   //styleIt->getStyles().insert(styleAss);
	   isi->setStyles(psa);
	   // associate created ro representationIte

	   isi->setItem(representationItem);
